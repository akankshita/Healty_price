...
# if you are using in local system you need to comment this
	 def ssl_required?
	...
	def index
		#=================================================
		if session[:doctor] != nil && session[:doctor].enabled == 1
			redirect_to '/doctor_section'
		elsif params[:doctor] == nil				# Original Form Load
			if session[:doctor] == nil && params[:step][0, 4] == 'step' && params[:step] != 'step1'	#once the step 1 is completed, one cannot go back to step 1
				...
				@selected_specialties = Specialty.find_by_sql("SELECT id, name, description FROM specialties, doctor_specialties " +
													...
													"AND doctor_id = " + session[:doctor].id.inspect +
													" ORDER BY name")
				@services = Specialty.find_by_sql("SELECT specialty_services.id AS specialty_service_id, specialty_services.specialty_id AS specialty_id, service_id, service_name, service_description, doctor_price " +
				...
				sel_services = Specialty.find_by_sql("SELECT specialty_service_id FROM doctor_services WHERE doctor_id = " + session[:doctor].id.inspect)
				...
				@insurance_carriers = Specialty.find_by_sql("SELECT insurance_company_name " +
												"FROM insurance_carriers " +
												"WHERE doctor_id = " + session[:doctor].id.inspect)
				@hospital_affiliations = Specialty.find_by_sql("SELECT usage_percentage, hospital " +
												"FROM hospital_affiliations " +
												"WHERE doctor_id = " + session[:doctor].id.inspect)
				@doctor_references = Specialty.find_by_sql("SELECT first_name, last_name, phone " +
												"FROM doctor_references " +
												"WHERE doctor_id = " + session[:doctor].id.inspect)
				@selected_services = Specialty.find_by_sql("SELECT specialty_id, service_name " +
												"FROM doctor_services, specialty_services " +
			...
			if params[:step] == 'step1'
				...
				if @error == ""
					...
					@doctor.notification_email = params[:doctor][:email]
				...
				if params[:doctor][:notification_email] == "" || !params[:doctor][:notification_email].match(/^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i)
					@error += "<li>Please enter a valid notification email address.</li>"
					@errors["notification_email"] = "Please enter a valid notification email address."
				else
				...
				session[:doctor].residency_training		= params[:doctor][:residency_training]
				session[:doctor].fellowship_training		= params[:doctor][:fellowship_training]
				...
				if @doctor_description == "" || @doctor_description == nil
					...
					@errors["description"] = "You have entered a description longer than 700 characters. Any text after 700 characters will be truncated."
				...
				insurance_carriers.each do |ic|
					if ic[1] != ""
						sql = "INSERT INTO insurance_carriers(doctor_id, insurance_company_name) " +
							"VALUES(" + session[:doctor].id.inspect + ", '" + ic[1].gsub("'", "''") + "')"
						ActiveRecord::Base.connection.execute(sql)
				...
				if params[:agree_to_terms] == nil
					...
					#---------------------------------------
					@error += "<li>You must agree to the HealthyPrice terms and conditions before you can proceed.<br/>Please check the box to accept the terms and conditions.</li>"
				end
				#--------------------------------------------------------------------------------------------------------
				@error += "</ul>"
				@error = @error.gsub("<ul></ul>", "")
